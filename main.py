...
def HermitePolynomialsUsingDividedDifferences(table,derivativesTable,value):
    """

    :param table: table of x points and y values
    :param derivativesTable: table of x points and y' values (derivatives table by x ,y)
    :param value: x value that we want to calculate his y value
    ...
    k=2 #for for loop
    p=2 #for for loop
    sol=(table[0][1])+((derivativesTable[0][1])*(value-table[0][0])) #the solution
    for i in range(len(arr[0])):#calculate hermit polynomial
        mul=1
        if i < len(arr[0])-2:
            for j in range(k):
                mul=mul*(value-tempArr[j][0])
            sol+=arr[0][p]*mul
            j=0
            p+=1
            if i!= (len(arr[0])-1):
    ...
    print("{0}  + {1} * {2} ".format(tempArr[0][1],derivativesTable[0][1],(math.ceil((value-table[0][0])*1000)/1000)),end=" ")
    for i in range(len(arr[0])):  # calculate hermit polynomial
        mul = 1
        if i < len(arr[0]) - 2:
            for j in range(k):
    ...
    print(" ")
    print("\nH{0}({1}) = {2}".format(rows-1,value,sol))
    return sol
